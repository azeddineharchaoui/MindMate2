...
if IN_COLAB:
    ...
    subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', 
                    ...
                    'pyngrok', 'ollama', 'python-dotenv', 'openai-whisper', 
...
class Config:
    ...
    NGROK_AUTH_TOKEN = os.getenv("NGROK_AUTH_TOKEN", "2zUwXlN5lG92enq4KlJJXilF4g9_2thHAMaq6BGMfpbWaeK8c")
            ...
            try:
                with self.db_lock:
                    ...
                    try:
                        ...
                        if "database is locked" in str(e) and retry_count < max_retries - 1:
                            logger.warning(f"Database locked during user creation (attempt {retry_count+1}/{max_retries}), retrying...")
                            retry_count += 1
                            delay = self.base_retry_delay * (2 ** retry_count)  # Exponential backoff
                            time.sleep(delay)
                        else:
...
def configure_cors():
    ...
    @app.route('/api/ngrok-check', methods=['GET', 'OPTIONS'])
    def ngrok_check():
        """
        Special endpoint that returns a simple JSON response to confirm ngrok authorization
        This helps the frontend verify authorization worked correctly
        """
        return jsonify({
            'ngrok_authorized': True,
            'status': 'ok',
            'timestamp': datetime.now().isoformat()
        })
    
    # Add a before_request handler to detect and handle ngrok auth bypassing
    # Add a flag to detect recursion and prevent it
    _handling_request = False
    
    @app.before_request
    def handle_ngrok_bypass():
        """Detect ngrok authorization requests and handle them by returning JSON responses"""
        # Skip if we're already handling a request (prevents recursion)
        if getattr(handle_ngrok_bypass, '_handling_request', False):
            return None
        
        # Skip for OPTIONS requests which are handled separately
        if request.method == 'OPTIONS':
            return None
        ...
        # Set recursion flag
        handle_ngrok_bypass._handling_request = True
        
        try:
            # Check headers for ngrok-specific patterns in User-Agent
            user_agent = request.headers.get('User-Agent', '').lower()
            referrer = request.headers.get('Referer', '').lower()
            
            # Look for ngrok or tunnel in URL path, User-Agent, or referrer
            ngrok_indicators = [
                '/ngrok-' in request.path.lower(),
                '/tunnel' in request.path.lower(),
                'ngrok' in request.path.lower(),
                'authorize' in request.path.lower(),
                'ngrok' in user_agent,
                'tunnel' in user_agent,
                'ngrok' in referrer
            ]
            ...
            if request.path == '/favicon.ico':
                try:
                    return send_file('frontend/public/favicon.ico')
                except:
                    return '', 204
            
            # If any ngrok indicators are found, return a clean JSON response
            if any(ngrok_indicators):
                logger.info(f"Ngrok auth/inspection detected: {request.path}")
                logger.info(f"User-Agent: {user_agent}")
                
                # For API endpoints, return special JSON
                if request.path.startswith('/api/'):
                    # Return JSON appropriate for the endpoint type
                    if request.path == '/api/status':
                        return jsonify({
                            "status": "active",
                            "is_valid_json": True,
                            "ngrok_auth_required": False,  # Don't trigger auth flow again
                            "timestamp": datetime.now().isoformat()
                        })
                    elif request.path == '/api/user/profile':
                        return jsonify({
                            "user_id": "temporary_user",
                            "therapy_preferences": {"preferred_approach": "cbt"},
                            "session_stats": {"total_sessions": 0},
                            "mood_trends": {"average_mood": 5.0},
                            "is_valid_json": True,
                            "ngrok_inspection": True,
                            "timestamp": datetime.now().isoformat()
                        })
                    elif request.path == '/api/therapy/modules':
                        # Return minimal modules data with safe fallback
                        fallback_modules = {
                            "cbt": {"name": "Cognitive Behavioral Therapy"},
                            "mindfulness": {"name": "Mindfulness"},
                            "dbt": {"name": "Dialectical Behavior Therapy"}
                        }
                        return jsonify({
                        ...
                        # Generic JSON for any other API endpoint
                        return jsonify({
                            "status": "ok",
                    ...
                    # For non-API endpoints, we can still return JSON
                    # This prevents ngrok authorization pages from breaking the frontend
                    return jsonify({
                        "status": "ngrok_auth_required",
                        "message": "This appears to be an ngrok authorization request. The system is handling it automatically.",
                        "is_valid_json": True,
                        "timestamp": datetime.now().isoformat()
                    })
                
            # Only check content type for POST requests with content
            if request.method == 'POST' and request.content_length:
                content_type = request.headers.get('Content-Type', '')
                # If it's expecting to send JSON but isn't, return a clear error
                if 'application/json' not in content_type and request.content_length > 0:
                    logger.warning(f"Request with invalid Content-Type: {content_type}")
                    response = jsonify({
            ...
            # Special handling for key API endpoints to ensure they never fail
            if request.path == '/api/user/profile' and request.method == 'GET':
                # Check auth and set user_id for use in the endpoint
                auth_header = request.headers.get('Authorization', '')
                if auth_header.startswith('Bearer '):
                    token = auth_header[7:]  # Remove 'Bearer ' prefix
                    try:
                        # Try to extract user_id from token, with fallback
                        user_id = user_manager.validate_session_token(token) or "temp_user"
                        request.user_id = user_id
                    except:
                        request.user_id = "temp_user"
                else:
                    request.user_id = "temp_user"
            
            # No special handling needed, continue with normal request processing
            return None
        
        finally:
            # Always reset the recursion flag
            handle_ngrok_bypass._handling_request = False
    ...
    def add_cors_headers(response):
        ...
        response.headers['Access-Control-Allow-Headers'] = 'Content-Type,Authorization,X-Requested-With,Cache-Control,Accept,Pragma'
        response.headers['Access-Control-Allow-Methods'] = 'GET,POST,PUT,DELETE,OPTIONS'
        # Ensure content type is always set to JSON for API endpoints
        if request.path.startswith('/api/') and not response.headers.get('Content-Type'):
            response.headers['Content-Type'] = 'application/json'
        return response

    # Proper OPTIONS handler that doesn't redirect
    ...
    def options_handler(path):
        ...
        # Always use wildcard for better compatibility
        response.headers.add('Access-Control-Allow-Origin', '*')
        ...
        # For wildcard origins, don't set credentials
...
def status():
    ...
    try:
        # Check for HTML indicators in request (might be ngrok trying to auth)
        user_agent = request.headers.get('User-Agent', '').lower()
        if 'ngrok' in user_agent or request.path.lower() != '/api/status':
            # This might be ngrok trying to authorize - return special response
            special_response = {
                "status": "ngrok_auth_required",
                "message": "This appears to be an ngrok authorization request",
                "is_valid_json": True,
                "timestamp": datetime.now().isoformat()
            }
...
def enhanced_transcribe():
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
            audio_file.save(tmp.name)
            result = app_globals.audio_service.transcribe_with_emotion(tmp.name)
...
# ========================
# UPDATED SYSTEM STARTUP SECTION
# ========================
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    os.environ["NGROK_AUTH_TOKEN"] = "2zUwXlN5lG92enq4KlJJXilF4g9_2thHAMaq6BGMfpbWaeK8c"
    from pyngrok import ngrok, conf
    
    # Set up CORS headers directly without creating a separate function
    # This avoids potential conflicts with existing route handlers
    
    # CORS headers are added by the configure_cors() function
    
    
    
    try:
        # Configure ngrok with improved settings
        conf.get_default().auth_token = os.environ["NGROK_AUTH_TOKEN"]
        
        # Set region (for performance)
        conf.get_default().region = "eu"
        
        # Create a unique tunnel ID and save it for reference
        os.environ["NGROK_TUNNEL_ID"] = str(uuid.uuid4())[:8]
        logger.info(f"✅ Ngrok configured with tunnel ID: {os.environ['NGROK_TUNNEL_ID']}")

        # Define port (make sure it's an integer and used by your Flask app)
        server_port = getattr(Config, "SERVER_PORT", 5000)

        # Configure tunnel options for better stability
        ngrok_options = {
            "addr": server_port, 
            "proto": "http"
        }
        
        # Initialize public_url to avoid reference errors
        public_url = None
        
        # Use a phased approach to establish the tunnel
        try:
            # Simplest attempt first - just connect with minimal options
            logger.info("Attempting simple ngrok connection first...")
            public_tunnel = ngrok.connect(server_port)
            public_url = public_tunnel.public_url
            logger.info(f"✅ Ngrok tunnel established successfully: {public_url}")
        except Exception as simple_error:
            logger.warning(f"Simple connection failed: {simple_error}")
            
            try:
                # Try with explicit options
                logger.info(f"Trying with explicit options: {ngrok_options}")
                public_tunnel = ngrok.connect(**ngrok_options)
                public_url = public_tunnel.public_url
                logger.info(f"✅ Ngrok tunnel established with options: {public_url}")
            except Exception as option_error:
                logger.error(f"Connection with options failed: {option_error}")
                
                # Final attempt - try with a different auth method
                try:
                    # First make sure any existing tunnels are closed
                    try:
                    ...
                    logger.info("Final attempt with different configuration...")
                    ngrok.set_auth_token(os.environ["NGROK_AUTH_TOKEN"])
                    public_tunnel = ngrok.connect(server_port)
                    public_url = public_tunnel.public_url
                    logger.info(f"✅ Ngrok tunnel finally established: {public_url}")
                except Exception as final_error:
                    logger.error(f"❌ All ngrok tunnel attempts failed: {final_error}")
                    logger.warning("⚠️ The API will only be accessible locally")
                    public_url = f"http://localhost:{server_port}"
        
        # Save the URL for future reference
        if public_url:  # Make sure we have a URL before trying to write it
            with open("ngrok_url.txt", "w") as f:
                f.write(f"NGROK_URL={public_url}\n")
                f.write(f"TUNNEL_ID={os.environ.get('NGROK_TUNNEL_ID', 'unknown')}\n")
                f.write(f"STARTED_AT={datetime.now().isoformat()}\n")
            
            logger.info(f"🌐 Public URL: {public_url}")
            logger.info(f"⚠️ Important: If you see ngrok auth pages in the frontend, visit this URL directly first")
            logger.info(f"🔧 Local Port: {server_port}")
        else:
            # Create a fallback URL for local development
            public_url = f"http://localhost:{server_port}"
            logger.info(f"Using fallback local URL: {public_url}")

    except Exception as e:
        logger.error(f"⚠️ Ngrok tunnel setup failed: {e}")
        logger.warning(f"🔧 Server will run on Local Port: {getattr(Config, 'SERVER_PORT', 5000)}")
        public_url = f"http://localhost:{getattr(Config, 'SERVER_PORT', 5000)}"

    # Run a quick health check on key API endpoints
    try:
        # Use a separate thread to test key API endpoints
        def api_health_check():
            try:
                ...
                base_url = f"http://localhost:{getattr(Config, 'SERVER_PORT', 5000)}"
                endpoints = [
                    '/api/status',
                    '/api/user/profile',  # This will fail without auth but should return JSON
                    '/api/therapy/modules',
                ...
                for endpoint in endpoints:
                    ...
                    try:
                        ...
                        # For API endpoints, verify response is JSON
                        if endpoint.startswith('/api/'):
                            if 'application/json' in content_type:
                                ...
                                try:
                                    ...
                                    if endpoint == '/api/status' and data.get('status') == 'active':
                                        logger.info("   Status endpoint returning correct active status")
                                except Exception as parse_err:
                                    logger.warning(f"⚠️ {endpoint}: Response claimed to be JSON but couldn't parse: {parse_err}")
                            else:
    ...
    try:
        # Use the global detection variable
        is_colab = globals().get('IN_COLAB', False)
        
        # Make sure public_url is defined even if ngrok setup failed
        if not locals().get('public_url'):
            public_url = f"http://localhost:{getattr(Config, 'SERVER_PORT', 5000)}"
            logger.warning(f"No public URL available, using local URL: {public_url}")
        ...
        try:
            ...
            logger.info("� Install 'qrcode' package for QR code generation")
            
        # Start the Flask app
        logger.info("🚀 Starting Flask server...")
        logger.info(f"📝 Check '{os.path.abspath('ngrok_url.txt')}' for the ngrok URL if needed")
        app.run(host="0.0.0.0", port=Config.SERVER_PORT, debug=False, use_reloader=False)
    except NameError as ne:
        if "get_ipython" in str(ne):
            # Not in a notebook environment
            logger.info("🖥️ Running in standard Python environment")
            app.run(host="0.0.0.0", port=Config.SERVER_PORT, debug=False, use_reloader=False)
        else:
            logger.error(f"❌ NameError: {ne}")
    except Exception as e:
