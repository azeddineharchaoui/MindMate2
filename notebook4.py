...
def start_cloudflared_tunnel(port: int) -> str:
    ...
    try:
        ...
        if public_url:
            ...
            logger.error("❌ Failed to get public URL from cloudflared")
            return None
...
if __name__ == "__main__":
    ...
    public_url = None
    if IN_COLAB:
        # Use cloudflared for tunneling in Colab
        public_url = start_cloudflared_tunnel(Config.SERVER_PORT)
    else:
        # For local environment, use local URL
        public_url = f"http://localhost:{Config.SERVER_PORT}"
    ...
    if public_url:
        ...
        logger.info(f"🔧 Local URL: http://localhost:{Config.SERVER_PORT}")
    ...
    try:
        ...
        def api_health_check():
            try:
                ...
                base_url = public_url or f"http://localhost:{Config.SERVER_PORT}"
                endpoints = [
                    '/api/status',
                    ...
                    try:
                        ...
                        if endpoint.startswith('/api/'):
                            if 'application/json' in content_type:
                                logger.info(f"✅ Endpoint {endpoint}: {status} - JSON")
                            else:
                                logger.warning(f"⚠️ Endpoint {endpoint}: {status} - Non-JSON")
                    except requests.Timeout:
                        logger.error(f"❌ Endpoint {endpoint}: Timeout")
        ...
        logger.warning("Skipping API health check - requests module not available")